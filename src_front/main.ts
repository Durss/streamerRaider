...
IRCClient.instance.addEventListener(IRCEvent.MESSAGE, async (event:IRCEvent)=> {
	...
	&& chunks[0] == store.botCommand.toLowerCase().trim()
	&& Utils.getRouteMetaValue(router.currentRoute.value, "lightMode") === true) {
		...
		try {
			...
			if(!store.botDescriptionFallback) return;
		...
		if(description?.length > 0) {
			...
			let chatMessage = store.botText.replace(/\{DESCRIPTION\}/gi, description);
			...
			let chatMessage = store.botText.replace(/(.*?\{PSEUDO\}[^ ]{0,2}).*/gi, "$1");
